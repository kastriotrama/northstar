...
- Made the internal TecDoc license reference optional at the user's direction. Missing values are explicitly persisted as `not_provided`; source version, format, path, checksum, counts, and row evidence remain required.
- Recorded the stakeholder-confirmed TecDoc hierarchy: Table 100 manufacturer and capability groups, Table 110 model-series folders under PC/CV/LCV, Table 120 passenger-car KTypes as actual Level 3 objects, and Table 125-to-155 engine allocation. Display-only group folders remain evidence rather than canonical graph entities.
- Implemented TecDoc 2.7 fixed-width parsers for Tables 012/100/110/120/125/155 and validated the complete 0326 hierarchy: 72,570 active KTypes, 584 manufacturers, 11,462 model series and 17,173 referenced engines. Multiple engines and country applicability remain lossless; 14,843 KTypes have no engine link and one link targets a deleted-but-present engine.
- Added multiplicity-safe PostgreSQL engine-relationship candidates and guarded Neo4j promotion. A 1,000-real-KType batch stored 1,256 relationships (216 ambiguous KTypes), reran with zero duplicates, and the real graph test confirmed one-edge idempotency plus transactional rejection of conflicting engines.
- Added official key-table fuel interpretation and conservative canonical promotion. A controlled local run promoted 1,000 unambiguous KTypes; 40,965 of 72,570 are currently eligible. Variant candidates preserve manufacturer/model source keys while awaiting missing platform Tables 714/715, so no unsupported graph relationship is invented.
- Added a searchable TecDoc frontend tab showing the 1,000 promoted KTypes, canonical engine/vehicle facts, promotion gates, source keys and row evidence. Backfilled the existing local promotion batch with its engine/manufacturer/model source keys; frontend search and detail rendering were browser-verified without console errors.
