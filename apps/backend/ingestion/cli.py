...
from ingestion.tecdoc.gap_suggestion_migrations import (
    run_tecdoc_gap_suggestion_migrations,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    gap_list_parser = subparsers.add_parser(
        "list-tecdoc-gaps",
        help=(
            "List TecDoc values the newest sealed rule version left unmapped, "
            "highest support first, each with the vocabulary and naming "
            "convention its field already uses. Read-only. Values a reviewer "
            "has already ruled, and values already suggested, are omitted so a "
            "loop converges instead of re-proposing the same work."
        ),
    )
    gap_list_parser.add_argument(
        "--limit", type=int, default=25, help="Most gaps to list."
    )
    gap_list_parser.add_argument(
        "--include-suggested",
        action="store_true",
        help="Also list gaps a suggestion already answers.",
    )
    gap_list_parser.add_argument(
        "--reuse-only",
        action="store_true",
        help=(
            "Only report what the deterministic reuse matcher can answer by "
            "itself, without judgement."
        ),
    )

    suggestion_parser = subparsers.add_parser(
        "record-tecdoc-suggestion",
        help=(
            "Record one proposed answer for an unmapped TecDoc value. Writes to "
            "the suggestion table; with --auto-apply-at it may also write the "
            "live ruling, attributed to --suggested-by."
        ),
    )
    suggestion_parser.add_argument("--canonical-field", required=True)
    suggestion_parser.add_argument("--source-term", required=True)
    suggestion_parser.add_argument(
        "--decision", choices=("accepted", "excluded"), default="accepted"
    )
    suggestion_parser.add_argument(
        "--suggested-value",
        default=None,
        help="Canonical target. Omit for an 'excluded' suggestion.",
    )
    suggestion_parser.add_argument(
        "--origin",
        choices=("reuse", "mint", "exclude"),
        required=True,
        help=(
            "'reuse' names a term the vocabulary already holds; 'mint' proposes "
            "a new one and is checked against the field's convention; 'exclude' "
            "claims the value is out of scope and pairs with --decision excluded."
        ),
    )
    suggestion_parser.add_argument(
        "--rationale", required=True, help="Why this value. Stored for the reviewer."
    )
    suggestion_parser.add_argument("--key-table", default=None)
    suggestion_parser.add_argument("--support", type=int, default=0)
    suggestion_parser.add_argument(
        "--confidence", type=float, required=True, help="0.0 to 1.0."
    )
    suggestion_parser.add_argument(
        "--suggested-by", required=True, help="Agent identity accountable for it."
    )
    suggestion_parser.add_argument(
        "--auto-apply-at",
        type=float,
        default=None,
        help=(
            "Confidence at or above which the suggestion also writes the live "
            "ruling. Omitted means suggest only."
        ),
    )

    apply_parser = subparsers.add_parser(
        "apply-tecdoc-suggestions",
        help=(
            "Promote recorded suggestions at or above a confidence into live "
            "TecDoc rulings. Rows already applied are left alone."
        ),
    )
    apply_parser.add_argument(
        "--at-least", type=float, required=True, help="Confidence bar, 0.0 to 1.0."
    )
    apply_parser.add_argument(
        "--applied-by", required=True, help="Who is accountable for promoting them."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "list-tecdoc-gaps":
        datastores = DatastoreClients.from_settings(settings)
        try:
            with datastores.postgres.connect() as connection:
                run_tecdoc_gap_suggestion_migrations(connection)
                rule_version = newest_sealed_version(connection)
                if rule_version is None:
                    print(json.dumps({"rule_version": None, "gaps": []}, sort_keys=True))
                    return 0
                gaps = read_open_gaps(
                    connection,
            ...
            logger.error(
                "TecDoc gap listing stopped safely",
                extra={"error_code": type(error).__name__},
            )
            return 1

        answered = suggest_all(gaps, (ReuseSuggester(),))
        listed = []
        for gap, proposal in answered:
            if args.reuse_only and proposal is None:
                continue
            entry: dict[str, Any] = {
                ...
                "support": gap.support,
                "context": field_context(gap.canonical_field),
            }
    ...
    if args.command == "record-tecdoc-suggestion":
        suggestion = Suggestion(
            ...
            convention=field_context(args.canonical_field)["convention"],
        ...
        try:
            with datastores.postgres.connect() as connection:
                run_tecdoc_gap_suggestion_migrations(connection)
                recorded = store_suggestion(
                    connection,
                    suggestion,
                    auto_apply_at=args.auto_apply_at,
                    applied_by=f"agent:{args.suggested_by}",
                )
        except Exception as error:  # noqa: BLE001
            logger.error(
                "TecDoc suggestion refused",
                extra={"error_code": type(error).__name__, "reason": str(error)},
            )
            return 1
        print(json.dumps(recorded, sort_keys=True))
        return 0

    if args.command == "apply-tecdoc-suggestions":
        datastores = DatastoreClients.from_settings(settings)
        try:
            with datastores.postgres.connect() as connection:
                run_tecdoc_gap_suggestion_migrations(connection)
                promoted = apply_pending(
                    connection,
            ...
            logger.error(
                "TecDoc suggestion promotion stopped safely",
                extra={"error_code": type(error).__name__},
            )
            return 1
        print(
            json.dumps(
                {"promoted": len(promoted), "rulings": list(promoted)},
                indent=2,
                sort_keys=True,
            )
        )
